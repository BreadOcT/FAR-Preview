...
import { CheckSquare, Square, Trash2, Heart } from 'lucide-react';
...
export const SavedItems: React.FC<SavedItemsProps> = ({ items, onDelete }) => {
    ...
    return (
        <div className="p-6 bg-[#FDFBF7] dark:bg-stone-950 min-h-screen animate-in fade-in">
            ...
            {isSelectionMode && selectedIds.size > 0 && (
                <button onClick={() => onDelete(selectedIds)} className="mb-4 text-red-500 text-sm font-bold flex items-center gap-1 w-full justify-end"><Trash2 className="w-4 h-4" /> Hapus ({selectedIds.size})</button>
            )}
            ...
            <div className="space-y-4">
                {filteredItems.length === 0 && (
                    <EmptyState 
                        ...
                        description="Belum ada makanan yang disimpan atau sesuai dengan filter."
                    />
                )}
                    ...
                    <div key={item.id} onClick={() => isSelectionMode && toggleSelection(item.id)} className={`flex gap-4 p-4 bg-white dark:bg-stone-900 rounded-xl border relative overflow-hidden ${selectedIds.has(item.id) ? 'border-orange-500 bg-orange-50 dark:bg-orange-900/10' : 'border-stone-200 dark:border-stone-800'}`}>
                        ...
                        <div className="flex-1">
                            ...
                            {!isSelectionMode && item.status === 'available' && (
                                <Button className="mt-2 w-auto h-8 text-xs px-4 rounded-full">Klaim</Button>
                            )}
                        </div>
                    </div>
                ))}
